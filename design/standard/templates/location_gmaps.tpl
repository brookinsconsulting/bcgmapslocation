 ...
 *}{* $parent.data_map|attribute(show,1)}
{$class|attribute(show,1)}
{$node|attribute(show,1)}
{$nodes|attribute(show,1) *}
{def $uniqueLocationEventScheduleClass=fetch( 'content', 'class', hash( 'class_id', ezini( 'GMapsLocationSettings', 'LocationEventScheduleClassID', 'bcgmapslocation.ini' ) ) )}
{if is_set( $nodes )}{def $children=$nodes}{else}{def $children = fetch( 'content','list',hash(  'parent_node_id',
                 $node.node_id,'priority',true() ) )}{/if}
{if count($children)|ne(0)}
{def $map_link_url=0
     $map_iframe_url=0}
...
{* $node.data_map.location.value|attribute(show,1) *}
...
window.onload=function(ev){
   //Run any onload that we found.
	    ...
	    var blueIcon = new GIcon(G_DEFAULT_ICON);
            // plainBlueIcon.image = "http://www.google.com/intl/en_us/mapfiles/ms/micons/blue-dot.png";
            blueIcon.image = "http://sfarchdiocese.org/media/images/mymaps-icons/parishes-gmarker.png";
...
$times=0}{if is_object($children)}{if and( $children.object.data_map.location.value.latitude|ne(''), $children.object.data_map.location.value.longitude|ne('') )}markers.push( new GMarker( new GLatLng( {$children.object.data_map.location.value.latitude}, {$children.object.data_map.location.value.longitude}{literal} ), markerOptions ) );
            infos[{/literal}{$count}] = '{literal}<div class="msinfotitle" dir="ltr" align="left" style="text-size:16px;font-weight:bolder">{/literal}<a style="text-decoration:none;color:#535548" href={$children.url|ezurl}>{$children.object.name}{literal}</a></div><div class="description" dir="ltr" align="left"><table><tbody><tr><td valign="top">{/literal}{if is_set( $children.object.data_map.image.value.data_map.image.value.original  )}<img src="{$children.object.data_map.image.value.data_map.image.value.original.url|ezurl('no')}">{else} {/if}</td><td valign="top">{$children.object.data_map.address.value}<br>{$children.object.data_map.city.value}, {$children.object.data_map.state.value} {$children.object.data_map.zip.value}<p>Phone: {$children.object.data_map.phone.value}</p><p>Email: <a target="_blank" href="mailto:{$children.object.data_map.email.value}">{$children.object.data_map.email.value}</a></p><p>Website: <a target="_blank" href="{$children.object.data_map.website.value}">{$children.object.data_map.website.data_text}</a></p><p>Contact: {$children.object.data_map.contact.value}</p>{literal}<p><b>Hours</b></p>{/literal}{set $language=0}{set $languageid=0}{set $times = fetch( 'content','list',hash( 'parent_node_id', $children.node_id,'priority',true() ) )}{foreach $times as $mass}{set $languageid=$mass.data_map.language.value.0}{set $language=$uniqueLocationEventScheduleClass.data_map.language.content.options.$languageid}{set $language=$language.name}<p>{$mass.name}: {$mass.data_map.time.value.timestamp|datetime('event_datetime')} ({$language})</p>{/foreach}{set $address=concat( $children.object.data_map.address.value, '+', $children.object.data_map.city.value, ',+', $children.object.data_map.state.value, '+', $children.object.data_map.zip.value, '+USA' )}{set $map_link_url=concat( $gmaps_directions_map_url_part0, $address, $gmaps_directions_map_url_part1 )}{if and( is_set( $children.object.data_map.scheduleanomalies ), $children.object.data_map.scheduleanomalies.value|ne('') )}<p>{$children.object.data_map.scheduleanomalies.value}</p>{/if}{literal}<p><b>Maps</b></p><p><a target="_blank" style="text-size:small; color: rgb(0,0,255); text-align: left" href="{/literal}{$map_link_url}">Get Directions to Parish</a></p>{set $address=concat( $children.object.data_map.address.value, '+', $children.object.data_map.city.value, ',+', $children.object.data_map.state.value, '+', $children.object.data_map.zip.value, '+USA' )}{set $address_map_url=concat( $gmaps_larger_map_url_part0, $address, $gmaps_larger_map_url_part1 )}<p><a target="_blank" style="text-size:small; color: rgb(0,0,255); text-align: left" href="{$address_map_url}">View larger map of Parish</a></p>{literal}</td></tr></tbody></table></div>';
	    ...
	    {set $count=$count|sum(1)}{/if}{else}{foreach $children as $child}{if and( $child.object.data_map.location.value.latitude|ne(''), $child.object.data_map.location.value.longitude|ne('') )}markers.push( new GMarker( new GLatLng( {$child.object.data_map.location.value.latitude}, {$child.object.data_map.location.value.longitude}{literal} ), markerOptions ) );
            infos[{/literal}{$count}] = '{literal}<div><div class="msinfotitle" dir="ltr" align="left" style="text-size:16px;font-weight:bolder">{/literal}<a style="text-decoration:none;color:#535548" href={$child.url|ezurl}>{$child.name}</a>{literal}</div><div class="description" dir="ltr" align="left"><table border=0 width=100%><tbody><tr><td valign="top">{/literal}{if is_set( $child.object.data_map.image.value.data_map.image.value.original )}<img src="{$child.object.data_map.image.value.data_map.image.value.original.url|ezurl('no')}">{else} {/if}</td><td valign="top">{$child.object.data_map.address.value}<br>{$child.object.data_map.city.value}, {$child.object.data_map.state.value} {$child.object.data_map.zip.value}<p>Phone: {$child.data_map.phone.value}</p><p>Email: <a target="_blank" href="mailto:{$child.data_map.email.value}">{$child.data_map.email.value}</a></p><p>Website: <a target="_blank" href="{$child.data_map.website.value}">{$child.data_map.website.data_text}</a></p><p>Contact: {$child.data_map.contact.value}</p>{literal}<p><b>Hours</b></p>{/literal}{set $language=0}{set $languageid=0}{set $times = fetch( 'content','list',hash( 'parent_node_id', $child.node_id,'priority',true() ) )}{foreach $times as $mass}{set $languageid=$mass.data_map.language.value.0}{set $language=$uniqueLocationEventScheduleClass.data_map.language.content.options.$languageid}{set $language=$language.name}<p>{$mass.name}: {$mass.data_map.time.value.timestamp|datetime('event_datetime')} ({$language})</p>{/foreach}{set $address=concat( $child.object.data_map.address.value, '+', $child.object.data_map.city.value, ',+', $child.object.data_map.state.value, '+', $child.object.data_map.zip.value, '+USA' )}{set $map_link_url=concat( $gmaps_directions_map_url_part0, $address, $gmaps_directions_map_url_part1 )}{if and( is_set( $child.data_map.scheduleanomalies ), $child.data_map.scheduleanomalies.value|ne('') )}<p>{$child.data_map.scheduleanomalies.value}</p>{/if}{literal}<p><b>Maps</b></p><p><a target="_blank" style="text-size:small;color: rgb(0,0,255); text-align: left" href="{/literal}{$map_link_url}">Get directions to location</a></p>{set $address=concat( $child.object.data_map.address.value, '+', $child.object.data_map.city.value, ',+', $child.object.data_map.state.value, '+', $child.object.data_map.zip.value, '+USA' )}{set $address_map_url=concat( $gmaps_larger_map_url_part0, $address, $gmaps_larger_map_url_part1 )}<p><a target="_blank" style="text-size:small; color: rgb(0,0,255); text-align: left" href="{$address_map_url}">View larger map of location</a></p>{literal}</td></tr></tbody></table></div></div>';
            map.addOverlay( markers[{/literal}{$count}{literal}] );{/literal}
...
{if is_set( $node.data_map.address )}
<b>Maps</b>
