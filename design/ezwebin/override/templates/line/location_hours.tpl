 ...
 *}{def $c=fetch( 'content', 'class', hash( 'class_id', ezini( 'GMapsLocationSettings', 'LocationHoursClassID', 'bcgmapslocation.ini' ) ) )
...
{foreach $childrenLocation as $child}{set $langid=$child.data_map.language.value.0}{set $lang=$c.data_map.language.content.options.$langid}{set $lang=$lang.name}{if $searchLang|eq($lang)}<p>{$child.name}: {$child.data_map.time.value.timestamp|datetime('event_datetime')}{if is_set( $child.data_map.end_time.value )} - {$child.data_map.end_time.value.timestamp|datetime('event_datetime')}{/if} ({$lang})</p>{/if}{/foreach}{undef $lang}{undef $langid}{/if}
...
{if and( $displayMapPointInfoMapLinksSetting|ne('disabled'), is_set( $node.parent.object.data_map.address ) )}
