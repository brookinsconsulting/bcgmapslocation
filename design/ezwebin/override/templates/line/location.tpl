...
{foreach $childrenLocations as $child}{set $langid=$child.data_map.language.value.0}{set $lang=$c.data_map.language.content.options.$langid}{set $lang=$lang.name}<p>{$child.name}: {$child.data_map.time.value.timestamp|datetime('custom', '%h:%i %a')} ({$lang})</p>{/foreach}{undef $lang}{undef $langid}
